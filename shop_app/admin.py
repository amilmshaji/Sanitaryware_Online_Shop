...
class ProductAdmin(admin.ModelAdmin):
    list_display = ['product_name', 'stock', 'is_available', 'is_featured', 'created_date', 'modified_date']
    list_filter = ['is_available', 'is_featured', 'created_date', 'modified_date']
    search_fields = ['product_name', 'description']
    ...
    def change_view(self, request, object_id, form_url='', extra_context=None):
        ...
        if product.stock < 3:
            ...
            message = ngettext(
                '%(count)d product has a stock level lower than 3. Please check the stock level.',
                '%(count)d products have a stock level lower than 3. Please check the stock levels.',
                product.stock
            ) % {'count': product.stock}
